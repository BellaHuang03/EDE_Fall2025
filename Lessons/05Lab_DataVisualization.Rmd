...
Face_temperature_day <- PeterPaul.chem.nutrients %>%
  filter(depth==0) %>% 
  ggplot(
    mapping = aes(
      x=daynum,
...
Temperature_date <- PeterPaul.chem.nutrients %>%
  ggplot(
    mapping=aes(x=sampledate,
                y=temperature_C,
                color=depth))+
      geom_point(size=0.5) +
      scale_color_viridis(
